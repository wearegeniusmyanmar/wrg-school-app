...
# =========================================================
# CUSTOM CSS (ORIGINAL FULL STYLES)
# =========================================================
st.markdown(
    """
<style>
...
def edit_timetable_dialog(row_id):
    ...
    levels_df = run_query("SELECT DISTINCT level AS \"Level\" FROM lessons ORDER BY level")
    levels = levels_df["Level"].dropna().astype(str).tolist()

    e_date = st.text_input("ရက်စွဲ (YYYY-MM-DD)", value=str(row["Date"]))
    e_type = st.selectbox(
        "ကျောင်းအမျိုးအစား",
        CLASS_TYPES,
        index=CLASS_TYPES.index(row["Class_Type"]) if row["Class_Type"] in CLASS_TYPES else 0,
    )
    ...
    e_zoom = st.text_input(
        "Zoom ID / Link",
        value=str(row["Zoom_ID"]) if pd.notna(row["Zoom_ID"]) else "",
    )

    if teachers:
        t_idx = teachers.index(row["Teacher_Name"]) if row["Teacher_Name"] in teachers else 0
        e_t = st.selectbox("Lead Teacher", teachers, index=t_idx)
    else:
        e_t = st.text_input("Lead Teacher", value=str(row["Teacher_Name"]))
    ...
    if levels:
        ...
        e_lvl = st.selectbox("Level ရွေးပါ", levels, index=l_idx)
    else:
        e_lvl = None

    if e_lvl:
        lessons_df = run_query(
            "SELECT lesson_topic AS \"Lesson_Topic\" FROM lessons WHERE level = %s ORDER BY id",
            (e_lvl,),
        )
        lessons = lessons_df["Lesson_Topic"].dropna().astype(str).tolist()
        if lessons:
            lsn_idx = lessons.index(row["Lesson_Topic"]) if row["Lesson_Topic"] in lessons else 0
            e_top = st.selectbox("Lesson ရွေးပါ", lessons, index=lsn_idx)
        else:
            e_top = None
    else:
        e_top = None
        ...
        if e_lvl and e_top and e_t:
            try:
                execute_query(
                    """
                    UPDATE timetable
                    SET date = %s, class_type = %s, class_name = %s, period = %s, zoom_id = %s,
                        teacher_name = %s, assistant_1 = %s, assistant_2 = %s,
                        lesson_level = %s, lesson_topic = %s
                    WHERE id = %s
                    """,
                    (
                        e_date,
...
def edit_student_dialog(row_id):
    row_df = run_query(
        """
        SELECT id, student_name AS "Student_Name", age AS "Age", class_type AS "Class_Type",
    ...
    with st.form("edit_stu_form"):
        ...
        e_age = st.number_input(
            ...
            min_value=3,
            max_value=80,
            value=int(row["Age"]) if pd.notna(row["Age"]) else 15,
        )
        e_parent = st.text_input(
            "မိဘ အမည်",
            value=str(row["Parent_Name"]) if pd.notna(row["Parent_Name"]) else "",
        )
        ...
        e_social = st.text_input(
            "မိဘ Social အကောင့်",
            value=str(row["Social_Account"]) if pd.notna(row["Social_Account"]) else "",
        )
        e_address = st.text_area(
            "လိပ်စာ",
            value=str(row["Address"]) if pd.notna(row["Address"]) else "",
        )
        ...
        if st.form_submit_button("💾 သိမ်းဆည်းမည်", type="primary", use_container_width=True):
            try:
                execute_query(
                    """
                    UPDATE students
                    SET class_type = %s, class_name = %s, student_name = %s, age = %s,
                        parent_name = %s, phone = %s, social_account = %s, address = %s
                    WHERE id = %s
                    """,
                    (
                        e_type,
...
def edit_teacher_dialog(row_id):
    row_df = run_query(
        "SELECT id, teacher_name AS \"Teacher_Name\", phone AS \"Phone\" FROM teachers WHERE id = %s",
        (row_id,),
    )
    ...
    with st.form("edit_teach_form"):
        ...
        if st.form_submit_button("💾 သိမ်းဆည်းမည်", type="primary", use_container_width=True):
            try:
                execute_query(
                    "UPDATE teachers SET teacher_name = %s, phone = %s WHERE id = %s",
...
def edit_lesson_dialog(row_id):
    row_df = run_query(
        "SELECT id, level AS \"Level\", lesson_topic AS \"Lesson_Topic\" FROM lessons WHERE id = %s",
        (row_id,),
    )
    ...
    with st.form("edit_lsn_form"):
        ...
        if st.form_submit_button("💾 သိမ်းဆည်းမည်", type="primary", use_container_width=True):
            try:
                execute_query(
                    "UPDATE lessons SET level = %s, lesson_topic = %s WHERE id = %s",
    ...
    with center_col:
        ...
        with st.form("clean_login_form"):
            role_choice = st.selectbox(
                ...
                ["Teacher (ဆရာ/မ)", "Admin (စီမံခန့်ခွဲသူ)"],
            )
...
if selected_menu == "📅 အပတ်စဉ် အချိန်ဇယား ရှာဖွေရန်":
    ...
    with c2:
        teacher_filter_df = run_query(
            "SELECT DISTINCT teacher_name AS \"Teacher_Name\" FROM teachers WHERE teacher_name IS NOT NULL ORDER BY teacher_name"
        )
        ...
        with st.form("adm_tt_form", clear_on_submit=True):
            ...
            with c1:
                in_date = st.date_input(
                    "ရက်စွဲ (Date)", date.today(), key="adm_in_date"
                ).strftime("%Y-%m-%d")
            ...
            with c3:
                ...
                lessons_under_level = (
                    lessons_under_level_df["Lesson_Topic"].dropna().astype(str).tolist()
                )
                in_top = st.selectbox(
                    "သင်ကြားရမည့် သင်ခန်းစာ",
                    lessons_under_level if lessons_under_level else ["သင်ခန်းစာမရှိပါ"],
                    key="adm_in_top",
                )
    ...
    if not tt_data.empty:
        ...
        with col_b1:
            if st.button(
                ...
                execute_query(
                    f"DELETE FROM timetable WHERE id IN ({','.join(['%s'] * len(selected_ids))})",
                    tuple(selected_ids),
                )
                run_query.clear()
    ...
    if not stu_df.empty:
        ...
        with col_sb1:
            if st.button(
                ...
                execute_query(
                    f"DELETE FROM students WHERE id IN ({','.join(['%s'] * len(sel_stu_ids))})",
                    tuple(sel_stu_ids),
                )
                run_query.clear()
...
# =========================================================
    ...
    st_classes = run_query(
        """
        SELECT DISTINCT class_type AS "Class_Type", class_name AS "Class_Name"
        FROM students
    ...
    if not st_classes.empty:
        ...
        with a2:
            class_types_available = sorted(st_classes["Class_Type"].dropna().unique().tolist())
            sel_at_type = st.selectbox("ကျောင်းအမျိုးအစား", class_types_available, key="at_tp_sel")
        with a3:
            filtered_cn = (
                st_classes[st_classes["Class_Type"] == sel_at_type]["Class_Name"]
                .dropna()
                .unique()
                .tolist()
            )
        ...
        stu_in_c = run_query(
            """
            SELECT student_name AS "Student_Name", phone AS "Phone"
            FROM students
            WHERE class_type = %s AND class_name = %s
        ...
        if not stu_in_c.empty:
            existing_att = run_query(
                """
                SELECT student_name AS "Student_Name", status AS "Status"
                FROM attendance
                WHERE date = %s AND class_type = %s AND class_name = %s
                """,
                (att_date, sel_at_type, sel_at_cls),
            )

            if not existing_att.empty:
                status_map = dict(zip(existing_att["Student_Name"], existing_att["Status"]))
            else:
                status_map = {}
            ...
            with st.form("daily_att_form"):
                ...
                if st.form_submit_button("💾 ကျောင်းခေါ်ချိန် သိမ်းဆည်းမည်", type="primary"):
                    ...
                    try:
                        ...
                        cur.executemany(
                            """
                            INSERT INTO attendance (date, class_type, class_name, student_name, phone, status)
                            VALUES (%s,%s,%s,%s,%s,%s)
                            """,
                            saved_att,
                        )
